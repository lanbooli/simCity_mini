...
// ── Embedded Metal shader source (fallback when .metallib is unavailable) ──
static const char* kEmbeddedMetalShaderSource = R"SHADER_SRC(
...
struct NormalRasterizerData { float4 position [[position]]; float2 texCoord; };
struct MaskedRasterizerData { float4 position [[position]]; float2 texCoord; float4 myPos; };

vertex MaskedRasterizerData VertShaderSrcSetupMask(
    uint vertexID [[vertex_id]],
    constant float2 *vertexArray [[buffer(0)]],
    constant float2 *uvArray [[buffer(1)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]]) {
    MaskedRasterizerData out;
    float2 vert = vertexArray[vertexID]; float2 uv = uvArray[vertexID];
    float4 pos = float4(vert.x, vert.y, 0.0, 1.0);
    out.position = uniforms.clipMatrix * pos;
    out.myPos = uniforms.clipMatrix * pos;
    out.texCoord = uv; out.texCoord.y = 1.0 - out.texCoord.y;
    return out;
}

fragment float4 FragShaderSrcSetupMask(
    MaskedRasterizerData in [[stage_in]],
    texture2d<float> tex [[texture(0)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]],
    sampler smp [[sampler(0)]]) {
    float isIn = step(uniforms.baseColor.x, in.myPos.x/in.myPos.w)
    ...
    return float4(uniforms.channelFlag * tex.sample(smp, in.texCoord).a * isIn);
}

vertex NormalRasterizerData VertShaderSrc(
    uint vertexID [[vertex_id]],
    constant float2 *vertexArray [[buffer(0)]],
    constant float2 *uvArray [[buffer(1)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]]) {
    ...
    float2 vert = vertexArray[vertexID]; float2 uv = uvArray[vertexID];
    out.position = uniforms.matrix * float4(vert.x, vert.y, 0.0, 1.0);
    out.texCoord = uv; out.texCoord.y = 1.0 - out.texCoord.y;
    return out;
}

vertex MaskedRasterizerData VertShaderSrcMasked(
    uint vertexID [[vertex_id]],
    constant float2 *vertexArray [[buffer(0)]],
    constant float2 *uvArray [[buffer(1)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]]) {
...
vertex NormalRasterizerData VertShaderSrcCopy(
    uint vertexID [[vertex_id]],
    constant float2 *vertexArray [[buffer(0)]],
    constant float2 *uvArray [[buffer(1)]]) {
    NormalRasterizerData out;
    out.position = float4(vertexArray[vertexID].x, vertexArray[vertexID].y, 0.0, 1.0);
    out.texCoord = uvArray[vertexID];
    return out;
}

fragment float4 FragShaderSrcCopy(
    NormalRasterizerData in [[stage_in]],
    texture2d<float> tex [[texture(0)]],
    sampler smp [[sampler(0)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]]) {
    return tex.sample(smp, in.texCoord) * uniforms.baseColor;
}

fragment float4 FragShaderSrc(
    NormalRasterizerData in [[stage_in]],
    texture2d<float> tex [[texture(0)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]],
    sampler smp [[sampler(0)]]) {
    float4 tc = tex.sample(smp, in.texCoord);
    tc.rgb = tc.rgb * uniforms.multiplyColor.rgb;
    tc.rgb = tc.rgb + uniforms.screenColor.rgb - (tc.rgb * uniforms.screenColor.rgb);
    float4 c = tc * uniforms.baseColor;
    return float4(c.rgb * c.a, c.a);
}

fragment float4 FragShaderSrcPremultipliedAlpha(
    MaskedRasterizerData in [[stage_in]],
    texture2d<float> tex [[texture(0)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]],
    sampler smp [[sampler(0)]]) {
    float4 tc = tex.sample(smp, in.texCoord);
    tc.rgb = tc.rgb * uniforms.multiplyColor.rgb;
    tc.rgb = (tc.rgb + uniforms.screenColor.rgb * tc.a) - (tc.rgb * uniforms.screenColor.rgb);
    return tc * uniforms.baseColor;
}

fragment float4 FragShaderSrcMask(
    MaskedRasterizerData in [[stage_in]],
    texture2d<float> tex0 [[texture(0)]],
    texture2d<float> tex1 [[texture(1)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]],
    sampler smp0 [[sampler(0)]], sampler smp1 [[sampler(1)]]) {
    float4 tc = tex0.sample(smp0, in.texCoord);
    tc.rgb = tc.rgb * uniforms.multiplyColor.rgb;
    tc.rgb = tc.rgb + uniforms.screenColor.rgb - (tc.rgb * uniforms.screenColor.rgb);
    float4 cf = tc * uniforms.baseColor;
    cf.rgb = cf.rgb * cf.a;
    float4 cm = (1.0 - tex1.sample(smp1, in.myPos.xy / in.myPos.w)) * uniforms.channelFlag;
    return cf * (cm.r + cm.g + cm.b + cm.a);
}

fragment float4 FragShaderSrcMaskInverted(
    MaskedRasterizerData in [[stage_in]],
    texture2d<float> tex0 [[texture(0)]],
    texture2d<float> tex1 [[texture(1)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]],
    ...
    tc.rgb = tc.rgb * uniforms.multiplyColor.rgb;
    tc.rgb = tc.rgb + uniforms.screenColor.rgb - (tc.rgb * uniforms.screenColor.rgb);
    float4 cf = tc * uniforms.baseColor; cf.rgb = cf.rgb * cf.a;
    float4 cm = (1.0 - tex1.sample(smp1, in.myPos.xy / in.myPos.w)) * uniforms.channelFlag;
    return cf * (1.0 - (cm.r + cm.g + cm.b + cm.a));
}

fragment float4 FragShaderSrcMaskPremultipliedAlpha(
    MaskedRasterizerData in [[stage_in]],
    texture2d<float> tex0 [[texture(0)]],
    texture2d<float> tex1 [[texture(1)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]],
...
fragment float4 FragShaderSrcMaskInvertedPremultipliedAlpha(
    MaskedRasterizerData in [[stage_in]],
    texture2d<float> tex0 [[texture(0)]],
    texture2d<float> tex1 [[texture(1)]],
    constant CubismShaderUniforms &uniforms [[buffer(2)]],
...
// ── Embedded Metal shader source (fallback when .metallib is unavailable) ──
    ...
    id<MTLLibrary> shaderLib = nil;
    if (libraryURL) {
        shaderLib = [device newLibraryWithURL:libraryURL error:nil];
    }
    if (!shaderLib) {
        NSLog(@"[CubismShader] metallib not found, compiling from embedded source");
        NSString *src = [NSString stringWithUTF8String:kEmbeddedMetalShaderSource];
        NSError *err = nil;
        shaderLib = [device newLibraryWithSource:src options:nil error:&err];
        if (!shaderLib) { NSLog(@"[CubismShader] Source compile failed: %@", err); return; }
    }
    if(!shaderLib)
    {
...
void CubismShader_Metal::GenerateBlendShader(CubismShaderSet* shaderSet, const csmString& vertShaderFileName, const csmString& fragShaderFileName, const csmString& vertShaderSrc, const csmString& fragShaderSrc, id<MTLDevice> device)
{
    ...
    {
        // Blend shader .metallib not available (CLI app, no bundle).
        // Drawables using advanced blend modes (beyond Normal/Add/Mult) will be
        // rendered with fallback. Haru model uses only basic blend modes, so this is safe.
        return;
    }
