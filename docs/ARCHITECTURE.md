...
| 客户端 | 类型 | 技术栈 |
|--------|------|--------|
| Web 前端 | 浏览器 SPA | Vanilla JS + CSS，由 FastAPI serve |
| 原生桌面宠物 | macOS 悬浮窗 | Swift + OC++ 桥接 + Live2D Cubism 5 + Metal |
| 桌面宠物 (旧) | macOS SwiftUI | 纯 Swift (PetApp.swift)，保留 |

详见 [LIVE2D_PET.md](LIVE2D_PET.md)。
